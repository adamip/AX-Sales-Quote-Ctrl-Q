Sub SalesQuoteFormatting()
    ...
    Dim WS_Count As Integer, FirstSheet_PageCount As Integer, I As Integer, LastRow As Integer, N As Integer
    ...
    For I = 1 To WS_Count ' process all sheets
        With ActiveWorkbook.Worksheets(I)
            ...
            Set UsedRange = .Range(.Range("C7"), .Range("AI7"))
            For Each Cell In UsedRange
                With Cell.MergeArea
                    If Not isQuote And Mid(Cell, 1) = "QUOTE" Then
                        isQuote = True
                    End If
                End With
            Next Cell
            ...
            If Not isQuote Then
                Set UsedRange = .Range(.Range("D7"), .Range("J9"))
                For Each Cell In UsedRange
                    With Cell.MergeArea
            ...
            If Not isQuote And Not isRequirements And Not isSnP Then
                Set UsedRange = .Range(.Range("F9"), .Range("J9"))
                For Each Cell In UsedRange
                    With Cell.MergeArea
            ...
            If Not isQuote And Not isRequirements And Not isSnP And Not isTnC Then
                Set UsedRange = .Range(.Range("C7"), .Range("F7"))
                For Each Cell In UsedRange
                    With Cell.MergeArea
                        If InStr(1, Cell, "Platinum") > 0 Then
                            isSupportPlatinum = True
                        End If
                        If InStr(1, Cell, "Gold") > 0 Then
                            isSupportGold = True
                        End If
                        If InStr(1, Cell, "Silver") > 0 Then
