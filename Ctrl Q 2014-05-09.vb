Sub SalesQuoteFormatting()
    ' SalesQuoteFormatting Macro                                                                            Adam Ip
    '                                                                                                       2013-04-09
    '
    ' This macro will auto set margins, scaling and other formatting features needed for proper Quote
    ' printing/PDFing.
    '
    ' Keyboard Shortcut: Ctrl+q
    '
    ' Copy Platinum, Gold, or Solver from template file Ctrl-Q.xls and paste to current Sales Quote
    '
    Dim isQuote As Boolean, isRequirements As Boolean, isTnC As Boolean, isSnP As Boolean
    Dim isSupportPlatinum As Boolean, isSupportGold As Boolean, isSupportSilver As Boolean
    ...
    Dim WS_Count As Integer, FirstSheet_PageCount As Integer, LastRow As Integer
    Dim I As Integer, N As Integer, ActiveSht As Integer
    Dim ActiveWB As String
    
    ' Associating Ctrl+Q to this Macro
    Application.OnKey "^q", "SalesQuoteFormatting"
    
    sngStart = Timer
    Application.ScreenUpdating = False
    
    For I = 1 To ActiveWorkbook.Worksheets.Count ' process all sheets
        ...
        With ActiveWorkbook.Worksheets(I)
            ...
            Set UsedRange = .Range(.Range("E7"), .Range("AF7"))
            ...
            With .PageSetup
                ...
                If isQuote Then
                    ...
                    Rem Previous RightFooter .RightFooter = "Page &P of " & FirstSheet_PageCount + WS_Count - 1
                    .RightFooter = ""
                ...
                ElseIf isRequirements Then
                    ...
                    Rem Previous .RightFooter   .RightFooter = "Page " & FirstSheet_PageCount + I - 1 & " of " & FirstSheet_PageCount + WS_Count - 1
                    .RightFooter = ""
                    .LeftFooter = "&7 - Prices exclude taxes, shipping costs, and traveling expenses" & Chr(10) + " - Please email Purchase Orders to: PurchaseOrders@adtechglobal.com or Fax to (678) 679-2347 - Attn: Sales" & Chr(10) + " - Options are not included in total."
                ...
                ElseIf isSupportPlatinum Or isSupportGold Or isSupportSilver Then
                    Workbooks.Open Filename:="C:\Sales Quote\Template\Ctrl-Q.xls"
                    If isSupportPlatinum Then
                        Sheets("Platinum2").Copy After:=Workbooks(ActiveWB).Sheets(ActiveSht)
                        Workbooks("Ctrl-Q.xls").Activate    're-gain focus
                        Sheets("Platinum1").Copy After:=Workbooks(ActiveWB).Sheets(ActiveSht)
                    ElseIf isSupportGold Then
                        Sheets("Gold2").Copy After:=Workbooks(ActiveWB).Sheets(ActiveSht)
                        Workbooks("Ctrl-Q.xls").Activate    're-gain focus
                        Sheets("Gold1").Copy After:=Workbooks(ActiveWB).Sheets(ActiveSht)
                    ElseIf isSupportSilver Then
                        Sheets("Gold2").Copy After:=Workbooks(ActiveWB).Sheets(ActiveSht)
                        Workbooks("Ctrl-Q.xls").Activate    're-gain focus
                        Sheets("Gold1").Copy After:=Workbooks(ActiveWB).Sheets(ActiveSht)
                        Workbooks("Ctrl-Q.xls").Activate    're-gain focus
                        Sheets("Silver2").Copy After:=Workbooks(ActiveWB).Sheets(ActiveSht)
                        Workbooks("Ctrl-Q.xls").Activate    're-gain focus
                        Sheets("Silver1").Copy After:=Workbooks(ActiveWB).Sheets(ActiveSht)
                    End If
                    Workbooks("Ctrl-Q.xls").Close
